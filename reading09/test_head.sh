...
for path in /etc/hosts /etc/passwd; do
    if ! diff -q <(head $path) <(./head.py $path); then
	echo "head test failed on $path!"
	exit 1
    fi
done

if ! diff -q <(cat /etc/passwd | head) <(cat /etc/passwd | ./head.py); then
...
for i in $(seq 10); do
    if ! diff -q <(cat /etc/passwd | head -n $i) <(cat /etc/passwd | ./head.py -n $i); then
